	...
	public:
		...
		void readTestData(char* fname){
			...
			if (writeModel){
				char* fname;
				if (loguai){
					fname = "../../data/EurLex.loguai";
				} else {
					fname = "../../data/EurLex.uai";
				}
				...
				for (int i = 0; i < T; i++){
					...
					if (loguai){
						fout << setprecision(10) << 0 << " " << -c_i << endl;
					} else {
						fout << setprecision(10) << 1 << " " << exp(-c_i) << endl;
					}
					fout << endl;
				}
				for (int k1 = 0; k1 < K; k1++){
					for (int k2 = k1+1; k2 < K; k2++){
						...
						if (loguai){
							fout << 0 << " " << 0 << endl;
							fout << setprecision(10) << 0 << " " << -c_e << endl;
						} else {
							fout << 1 << " " << 1 << endl;
							fout << setprecision(10) << 1 << " " << exp(-c_e) << endl;
						}
						fout << endl;
					}
				}
